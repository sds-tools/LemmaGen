...
Rule *CoverNodeAlg::GetBestRule(WordList::iterator &itStart, WordList::iterator &itEnd,
	...
	//in case of searching for rule (not preview-ing), something must be wrong
	//if we haven't find any. In this case we create default rule (""->"");
	if (!bPreview && rBestRule == NULL) {
		char* rule = new char[2];
		rule[0] = '\t';
		rule[1] = NULL;
		rBestRule = new Rule(rule);
	}
